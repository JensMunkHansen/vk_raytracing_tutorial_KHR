...
#include "_autogen/rtfailedsss.slang.h"
...
public:
  ...
  void createRayTracingPipeline() override
  {
    ...
    VkShaderModuleCreateInfo shaderCode = compileSlangShader("rtfailedsss.slang", rtfailedsss_slang);
    ...
    stages[eAnyHitShadow].pName = "rahitShadowMain";  // new
    ...
    // shadow any-hit shader (NEW hit group)
    group.type             = VK_RAY_TRACING_SHADER_GROUP_TYPE_TRIANGLES_HIT_GROUP_KHR;
    group.generalShader    = VK_SHADER_UNUSED_KHR;
    group.closestHitShader = VK_SHADER_UNUSED_KHR;  // shadow rays skip CHS
    group.anyHitShader     = eAnyHitShadow;         // <-- IMPORTANT
    shaderGroups.push_back(group);
    ...
    //    m_sbtGenerator.addIndices(rtPipelineInfo);
    m_sbtGenerator.reset();
    m_sbtGenerator.addIndex(nvvk::SBTGenerator::eRaygen, 0);
    m_sbtGenerator.addIndex(nvvk::SBTGenerator::eMiss, 1);
    m_sbtGenerator.addIndex(nvvk::SBTGenerator::eMiss, 2);
    m_sbtGenerator.addIndex(nvvk::SBTGenerator::eHit, 3);
    m_sbtGenerator.addIndex(nvvk::SBTGenerator::eHit, 4);
    // Creating the SBT
